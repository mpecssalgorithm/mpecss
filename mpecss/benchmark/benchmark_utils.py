"""Benchmark orchestration, result logging, and run provenance utilities."""
...
def run_single_problem_internal(
    ...
    row["seed"]            = seed
    row["wall_timeout_cfg"] = wall_timeout
    ...
    row["problem_size_mode"] = _classify_problem_size(n_x)
    ...
    row["time_bootstrap"] = res.get("bootstrap_time")
...
def _write_run_env(
    ...
    candidate_roots = [
        ...
        os.environ.get("MPECSS_REPO_DIR"),
        os.environ.get("GIT_REPO_DIR"),
        project_root_guess,
        os.getcwd(),
    ...
    for candidate in candidate_roots:
        ...
        if candidate in seen_candidates or not os.path.isdir(candidate):
            continue
        ...
        try:
            ...
            git_errors.append(f"{candidate}: {type(exc).__name__}: {exc}")
            continue
    else:
        env["git"]["capture_error"] = "; ".join(git_errors) if git_errors else "no git candidate directories"
